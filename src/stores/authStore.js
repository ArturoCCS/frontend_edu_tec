...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    async register(credentials) {
      try {
        const response = await axios.post(`${VITE_APP_URL_BACKEND}/auth/register`, credentials, { withCredentials: true });
    ...
    async confirmEmail(token) {
      try {
        const response = await axios.get(`${VITE_APP_URL_BACKEND}/auth/confirm/${token}`, { withCredentials: true });
        return { status: response.data.status, message: response.data.message };
      } catch (error) {
