<template>
  ...
  <select v-model="idCapitulo" @change="obtenerSecciones" required>
    <option v-for="capitulo in capitulos" :key="capitulo.ID_Capitulo" :value="capitulo.ID_Capitulo">{{ capitulo.Titulo }}</option>
  </select>
...
import { onMounted, ref } from 'vue'
  ...
  // Obtener capítulos al seleccionar un blog
  ...
  const crearBlog = async () => {
    try {
      ...
      tipo.value = 'simple'
      alert('Blog creado')
      obtenerBlogs() // Actualiza la lista de blogs después de crear uno
    } catch (err) {
      console.error(err)
  ...
  Orden: ordenCapitulo.value
      ...
      obtenerCapitulos() // Actualiza la lista de capítulos después de crear uno
    } catch (err) {
  ...
  // Cargar los blogs al montar el componente
  onMounted(() => {
    obtenerBlogs()
  })
  </script>
